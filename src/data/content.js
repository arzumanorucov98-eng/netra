export const companyInfo = {
  name: "Netra Marketing",
  phone: "+994519784946",
  instagram: "https://www.instagram.com/netra.marketing?igsh=MTMxYnRpamxhMWZxdQ%3D%3D&utm_source=qr",
  tiktok: "https://vt.tiktok.com/ZSQ3TcMd7/",
  whatsapp: "https://wa.me/994519784946",
  slogan: "Sosial Media Marketinq Sistemi Qururuq, Biznesləri Böyüdürük."
};

export const aboutText = [
  "Netra Marketing 5 illik təcrübəyə malik rəqəmsal marketinq agentliyidir. Biz bizneslər üçün sosial media marketinq sistemləri qurur, onların internetdə tanınmasını artırır və davamlı satış əldə etmələrinə kömək edirik.",
  "Məqsədimiz sadəcə paylaşım etmək deyil, biznesiniz üçün işləyən və nəticə gətirən marketinq sistemi qurmaqdır. Sosial media hesablarınızı aktiv saxlayır, hədəf auditoriyanıza çatdırır və reklam strategiyaları ilə yeni müştərilər qazanmağınıza kömək edirik.",
  "Netra Marketing olaraq sosial media idarəçiliyi, reklamların idarə olunması, video çəkiliş, dizayn xidmətləri və süni intellekt texnologiyaları ilə bizneslərin böyüməsinə dəstək veririk."
];

export const servicesList = [
  "Sosial Media İdarəçiliyi",
  "Instagram və Facebook Reklamları",
  "TikTok Marketinqi",
  "Professional Video Çəkiliş",
  "Reels və Video Montaj",
  "Post Dizaynları",
  "Story Hazırlanması",
  "AI (Süni İntellekt) Video Hazırlanması",
  "Brend İnkişafı",
  "Satış Strategiyalarının Qurulması",
  "Veb saytların hazırlanması"
];

export const packagesData = [
  {
    id: 1,
    name: "Brend Quruculuğu Paketi",
    price: "200 AZN",
    description: "Biznesinizi sosial mediada peşəkar şəkildə təqdim etmək üçün bütün səhifələrinizi sıfırdan qururuq.",
    features: [
      "Instagram və Facebook səhifələrinin yaradılması",
      "Peşəkar bioqrafiyanın hazırlanması",
      "Profil və loqo yerləşdirilməsi",
      "Highlight (Qapaq Hekayə) dizaynlarının hazırlanması",
      "Səhifənin korporativ rənglərinin və vizual üslubunun qurulması",
      "Axtarışlarda daha rahat görünmək üçün açar sözlərin və hashtagların optimizasiyası",
      "Əlaqə məlumatlarının düzgün yerləşdirilməsi",
      "İlk paylaşımlar üçün kontent istiqamətinin müəyyən edilməsi",
      "Müştəri etimadı yaradan peşəkar səhifə görünüşünün formalaşdırılması",
      "Brendinizin sosial mediada düzgün təqdim olunması"
    ],
    targetAudience: "Bu paket yeni fəaliyyətə başlayan və ya sosial media hesablarını peşəkar səviyyədə qurmaq istəyən bizneslər üçün nəzərdə tutulub."
  },
  {
    id: 2,
    name: "Start Paketi",
    price: "500 AZN / Ay",
    description: "Sosial mediada aktivliyə başlamaq üçün ideal paket.",
    features: [
      "Həftədə 1 dəfə video çəkiliş",
      "Aylıq 20 story paylaşımı",
      "Target reklam xidməti",
      "TikTok səhifəsinin idarə olunması",
      "Instagram səhifəsinin idarə olunması",
      "Facebook səhifəsinin idarə olunması"
    ]
  },
  {
    id: 3,
    name: "Standart Paket",
    price: "600 AZN / Ay",
    description: "Daha çox məzmun və dizayn ilə biznesinizi böyüdün.",
    features: [
      "Həftədə 1 dəfə video çəkiliş",
      "Həftədə 2 post dizaynı",
      "Aylıq 20 story paylaşımı",
      "Target reklam xidməti",
      "1 AI video hazırlanması",
      "TikTok, Instagram və Facebook səhifələrinin idarə olunması"
    ]
  },
  {
    id: 4,
    name: "Premium Paket",
    price: "800 AZN / Ay",
    description: "Geniş auditoriyaya çatmaq və yüksək fəallıq üçün.",
    features: [
      "Həftədə 2 dəfə video çəkiliş",
      "Aylıq 20 video paylaşımı",
      "Aylıq 30 story paylaşımı",
      "Həftədə 3 post dizaynı",
      "Target reklam xidməti",
      "2 AI video hazırlanması",
      "TikTok, Instagram və Facebook səhifələrinin idarə olunması"
    ],
    highlight: true
  },
  {
    id: 5,
    name: "VIP Paket",
    price: "900 AZN / Ay",
    description: "Maksimum nəticə və tam marketinq idarəçiliyi.",
    features: [
      "Həftədə 3 dəfə video çəkiliş",
      "Aylıq 30 video paylaşımı",
      "Aylıq 30 story paylaşımı",
      "Həftədə 3 post dizaynı",
      "Target reklam xidməti",
      "4 AI video hazırlanması",
      "TikTok, Instagram və Facebook səhifələrinin idarə olunması"
    ]
  }
];

export const partners = [
  {
    name: "Taxvibe",
    logo: "/partners/taxvibe.jpg",
    instagram: "https://www.instagram.com/taxvibe.az?igsh=MWh4ZWUyNG1vbnlsbQ%3D%3D&utm_source=qr"
  },
  {
    name: "Çiçək Təmizlik Şirkəti",
    logo: "/partners/cicek.jpg",
    instagram: "https://www.instagram.com/temizlik.xidmeti.cicek?igsh=Z2U1dzNmenVzOHQx&utm_source=qr"
  },
  {
    name: "ARV DOORS",
    logo: "/partners/arvdoors.jpg",
    instagram: "https://www.instagram.com/arvdoors.azerbaycan?igsh=MTIzZWM3azlhZWdybg%3D%3D&utm_source=qr"
  },
  {
    name: "Xəzər Stone",
    logo: "/partners/xezerstone.jpg",
    instagram: "https://www.instagram.com/xezer.stone?igsh=MWRxbDYwNTA0OW4ybQ%3D%3D&utm_source=qr"
  },
  {
    name: "VTB Kids Club",
    logo: "/partners/valideyn.jpg",
    instagram: "https://www.instagram.com/vtb_kids_club?igsh=Z2twejE0dm5zOHk4&utm_source=qr"
  }
];
